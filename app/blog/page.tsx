...
async function getcasestudies(): Promise<CaseStudy[]> {
  const res = await fetch(ADMIN_BASE_URL + "/api/case-studies", {
    headers: {
...
export default function BlogPage() {
  ...
  const [slider1, setSlider1] = useState<CaseStudy[]>([]);
  const [slider2, setSlider2] = useState<CaseStudy[]>([]);
  useEffect(() => {
    ...
    getcasestudies().then((data) => {
       ...
       if (data.length <= 2) {
          ...
          // Max 3-3 split
          setSlider1(data.slice(0, 2));
          setSlider2(data.slice(2, 4));
        } else {
          // More than 6 => 50%-50% split
          const mid = Math.floor(data.length / 2);

          setSlider1(data.slice(0, mid));
          setSlider2(data.slice(mid));
        }
      ...
      >
        <div className="container px-0">
          ...
          <div className="mb-12 md:mb-[30px] px-4 flex flex-col items-start text-left">
            ...
            <FadeUp delay={0.2}>
              <p className="text-[16px] md:text-[18px] lg:text-[20px] font-sans font-medium text-black max-w-[800px] leading-relaxed">
                A closer look at select transactions, highlighting strategy,
                structure, and execution across real-world deals.
              </p>
            </FadeUp>
          </div>
          ...
          <div className="relative">
            ...
            >
              {slider1.map((study: CaseStudy, idx: number) => (
                <SwiperSlide key={study.id}>
                  ...
                  { study.feature_image && (
                    <Image
                      src={ADMIN_BASE_URL+"/storage/" + study.feature_image || "/placeholder.jpg"}
                      alt={study.title}
                      fill
                    ...
                    <div className="relative z-20 w-full p-[24px] md:p-[34px] flex flex-col justify-end h-full mt-auto">
                      <Link href={study.slug} className="inline-block w-fit">
            ...
            >
              {slider2.map((study, idx) => (
                <SwiperSlide key={study.id}>
                  ...
                  >
                    {/* Image */}
                 { study.feature_image &&  <Image
                      src={ADMIN_BASE_URL+"/storage/" + study.feature_image}
                      alt={study.title}
                      fill
                      className="object-cover transition-transform duration-700 ease-in-out group-hover:scale-110"
                    />
        ...
        <div className="relative z-10 container">
          ...
          <div className="max-w-[710px] mx-auto">
            ...
            <FadeUp delay={0.2}>
              ...
              >
                Plain-English guidance on M&A strategy, legal structuring, and
                emerging issues shaping modern dealmaking.
              </p>
            </FadeUp>
          </div>
                  ...
                  <SwiperSlide key={blog.id}>
                    <div className="bg-white shadow-[2px_3px_6px_0px_#00000040] px-[20px] rounded-[40px] py-[32px] h-full">
                      <ImageReveal
                        delay={0.1}
                        direction="top"
                        className="relative md:h-[286px] h-[220px] rounded-[20px]"
                      >
                        {blog?.feature_image && (
                          <Image
                            src={
                              ADMIN_BASE_URL + "/storage/" + blog?.feature_image
                            }
                            alt={blog.title}
                            fill
                      ...
                      <div className="mt-[22px]">
                        <span className="text-black">April 26, 2026</span>
          ...
          <div className="max-w-[1045px] mx-auto mb-[40px]">
            ...
            <FadeUp delay={0.2}>
              <p className="text-[16px] lg:text-[20px] text-center text-black font-sans font-medium">
                Written summaries and key takeaways from the Merger Lawyer
                Insight Series—making complex discussions accessible and
                actionable.
              </p>
            </FadeUp>
          </div>
